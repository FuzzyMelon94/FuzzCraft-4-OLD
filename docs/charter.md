...
| 7 | QoL & Final Compat | 🔄 In Progress | v0.7.1. QoL mods added: RightClickHarvest, TorchMaster, Particle Effects, Naturally Trimmed, Trimmable Tools, Clumps, Reese's Sodium Options (restored), Collapsible Groups (JEI/CF). Curios slots configured (2 back/4 ring/4 charm). KubeJS work (caramel bridge, extruder recipes, difficulty scaling) deferred to B8. AU Tropicraft fix needs first-run config commit. Hybrid Aquatic re-add candidate (Lithostitched issue resolved per B7 sweep). Candlelight archived — removed from watchlist. |
---
## 8. Issue & Debug Log
