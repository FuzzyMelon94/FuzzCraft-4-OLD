...
| 6 | Building & Decoration | ✅ SP Tested | ~35 mods (v0.6.1–0.6.2). Full decoration suite: Macaw's (10 mods), Let's Do Beachparty + Furniture, Chipped, Rechiseled + Create, Framed Blocks, Supplementaries (pinned 3.6.4 — 3.6.8 CompatSodiumFluidRendererMixin crash on Sodium 0.6.13), Decorative Blocks Reborn, Dusty Decorations, Twigs, Handcrafted, Cluttered, Diagonal Fences/Walls/Windows, Every Compat Wood + Stone, Armor Statues, Dramatic Doors, MmmMmmMmmMmm. Candlelight: no 1.21.1 NeoForge port (watchlisted). Questing moved to B8. SP tested 2026-06-16. Server test to follow post-merge. Note: Chipped removed in Reduction Round 1 (texture atlas cost). |
| 8 | QoL Extras | ✅ Complete | 5 mods (v0.8.1). ToastBegone, NERB + OctoLib dep, Create: Additional Logistics, Distant Horizons 3.1.2-b (client-only). DH kept client-only due to server overhead constraints. SP tested 2026-07-02. Server test to follow post-merge. |
| — | Reduction Round 1 | ✅ Complete | v0.8.2–0.8.6. Performance trim pass targeting server CPU and client RAM. **Removed (10 mods):** Stellaris + Potentials (space dimension, non-essential, server worldgen cost), Dynamic Trees + DynamicTreesPlus + DT-BWG + DT-Terralith (per-tick tree simulation; heavy on weak hardware; partial coverage anyway — no compat for TF/Aether/Undergarden/Deeper&Darker/Bumblezone/Tropicraft), Alex's Mobs + Citadel (heaviest mob AI mod), Chipped (texture atlas bloat, ~800 block variants), Distant Horizons (killing client performance). **Added (5+1 dep):** EMI++ (collapsible groups), EMIffect, EMI Loot + Fzzy Config dep, EMI Ores, Let's Do EMI Compat. **Config:** README updated — G1GC replaces ZGC (DH gone; G1GC better at 4–6 GB), RAM recommendation dropped from 6–8 GB to 4–6 GB. SP + server tested 2026-07-03. |
| 7 | QoL & Final Compat | ✅ Complete | v0.7.1–0.7.4. Wave 1 (0.7.1): RightClickHarvest, TorchMaster, Particle Effects, Naturally Trimmed, Trimmable Tools, Clumps, Reese's Sodium Options (1.8.3), Collapsible Groups (JEI/CF). Curios slots (2 back/4 ring/4 charm). ModernFix dynamic_resources enabled. AU Tropicraft fix. Wave 2 (0.7.2–0.7.4): ~51 QoL mods — Dynamic FPS, GraveStone, Better Days, Polymorph, Carry On, Zume, Despawn Tweaks, BetterF3, Sound Physics, TrashSlot, Controlling + Searchables, FTB Ultimine + Create Ultimine (VeinMiner replaced), Trample No More, Packet Fixer, and more. Configs applied: Traveler's Titles (biome titles off), GraveStone (obituary off), Enhanced Attack Indicator (cleaner HUD), dodge rebound to R. Side fixes: Double Doors + RightClickHarvest corrected to `both`. Steam 'n' Rails updated 0.2.0 → 0.3.0-alpha.2 (PhantomSpriteManager crash). RRLS removed. CRLF/LF hash mismatch root-caused and fixed via `.gitattributes`. SP tested 2026-06-17. KubeJS deferred to B8. Server performance (keepAlive timeout on slow CPU) deferred to separate trim session. |
---
## 8. Issue & Debug Log
...
| 2026-06-16 | 5c | Almost Unified tag unification causes JEI to display Pam's HarvestCraft Pina Colada as the output for the Tropicraft cocktail recipe. The correct item is `tropicraft:cocktail` (crafted with pineapple cubes + coconut chunks + bamboo mug). Portal works correctly with the crafted item, but the JEI mismatch would mislead players. | Needs Almost Unified config exclusion for `tropicraft:cocktail` — deferred to B7 compat pass. |
| 2026-06-16 | 6 | Supplementaries 3.6.8 `CompatSodiumFluidRendererMixin` targets a `FluidRendererImpl` method signature not present in Sodium 0.6.13 — `InjectionError: Critical injection failure` crash at terrain loading (0/1 targets found, no refMap loaded). | Pinned Supplementaries to 3.6.4 (last stable build before Sodium compat API was updated). Re-evaluate when Sodium is upgraded past 0.6.13. |
| 2026-06-29 | 7 | GUI blank on every server reconnect — EMI panel, vanilla inventory items, and all HUD icons (JourneyMap, gamemode buttons) invisible but still interactable. 100% reproducible, first join unaffected. | ChatAnimation 1.1.3 mixin pushes PoseStack at `@At("HEAD")` in `ChatComponent.render()` but only pops at `@At("TAIL")`. On reconnect chat is empty/hidden, method returns early, pop never fires — PoseStack left corrupt for the session. Fixed by updating ChatAnimation 1.1.3 → 1.2.0 (GitHub issue #28, fix released 2026-06-27). Resolved v0.7.5. |
---
## 9. Handoff Protocol
